...
module vfabric_ldexp(clock, resetn, 
...
parameter FIFO_DEPTH = 64;
 ...
 assign is_fifo_stalled = !(is_fifo_a_valid & is_fifo_b_valid) | is_ldexp_stalled ;

 acl_fp_ldexp_hc ldexp_unit(.clock(clock), .resetn(resetn), 
   .dataa(dataa), .datab(datab), .valid_in(is_fifo_a_valid & is_fifo_b_valid), 
   .valid_out(o_dataout_valid), 
   .stall_in(i_stall), .stall_out(is_ldexp_stalled), .result(o_dataout));
