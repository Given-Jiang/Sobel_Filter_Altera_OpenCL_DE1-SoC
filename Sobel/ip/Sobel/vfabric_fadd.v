...
module vfabric_fadd(clock, resetn, 
...
parameter FIFO_DEPTH = 64;
 ...
 wire is_fifo_stalled, is_fadd_stalled;
 ...
 assign is_fifo_stalled =  ~(is_fifo_a_valid & is_fifo_b_valid) | is_fadd_stalled;

 acl_fp_custom_add_ll_hc fadd_unit(
        .clock(clock), .resetn(resetn), .dataa(dataa), .datab(datab), 
        .result(o_dataout), .valid_in(is_fifo_a_valid & is_fifo_b_valid), 
        .valid_out(o_dataout_valid), 
	.stall_in(i_stall), .stall_out(is_fadd_stalled));
